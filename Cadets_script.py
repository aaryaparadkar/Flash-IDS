...
# 
# This notebook evaluates Flash on the DARPA E3 Cadets dataset using an online GNN + Word2Vec pipeline.
# 
# ---
# 
# ## Two Usage Modes
# 
# **Mode A — Pipeline Orchestrator (recommended for your own CDM JSONL):**
# 1. Set your CDM JSONL file paths in the **CONFIGURATION** cell below.
# 2. Set `Train = True` and `USE_SYNTHETIC = False`.
# 3. Optionally set `SAMPLE_LINES` for a quick smoke test.
# 4. Execute all cells **Run All**. The `run_training_pipeline()` at the end handles everything: sampling, parsing, train/test split, Word2Vec, GNN training, and evaluation.
# 
# **Mode B — Step-by-step (original cell-by-cell flow):**  
# Execute cells sequentially and use `Train`, `USE_SYNTHETIC`, and interactive variables to control each stage manually.
# 
# ---
# 
# ## Input Format
# - **CDM JSONL**: One JSON object per line, each containing a `datum` with a CDM18 typed record.
# - Every line must include Event records (`com.bbn.tc.schema.avro.cdm18.Event`) for edge extraction.
# 
# ## Dataset Access:
# - Original Cadets dataset: [DARPA E3](https://drive.google.com/drive/folders/1QlbUFWAGq3Hpl8wVdzOdIoZLFxkII4EK)
# 
# ## Output
# - `cadets_train.txt` / `cadets_test.txt` — parsed edge TSVs (6 columns)
# - `trained_weights/cadets/word2vec_cadets_E3.model` — Word2Vec embeddings
# - `trained_weights/cadets/lword2vec_gnn_cadets{0..21}_E3.pth` — GNN snapshots
# - Precision/Recall/F1 metrics printed at the end
# 
...
def add_attributes(d, p):
    ...
    d = d.astype(str)
    result = d.merge(rdf, how='inner', on=['actorID','objectID','action','timestamp']).drop_duplicates()
    logger.info(f"Attribute enrichment: {len(d)} edges -> {len(result)} enriched")
    return result
...
if Train:
    ...
    df = add_attributes(df, TEST_JSONL_PATHS[0] if TEST_JSONL_PATHS else TRAIN_JSONL_PATHS[0] if TRAIN_JSONL_PATHS else "dummy")
...
if Train:
    word2vec = Word2Vec(sentences=phrases, vector_size=W2V_VECTOR_SIZE, window=W2V_WINDOW, min_count=1, workers=8, epochs=W2V_EPOCHS, callbacks=[saver, epoch_logger])
...
if Train:
    ...
    logger.info(f"Starting GNN training with {NUM_SNAPSHOTS} snapshots")
    for m_n in range(NUM_SNAPSHOTS):

      loader = NeighborLoader(graph, num_neighbors=[-1,-1], batch_size=GNN_BATCH_SIZE, input_nodes=mask)
      total_loss = 0
      for subg in loader:
          model.train()
          optimizer.zero_grad() 
      ...
      torch.save(model.state_dict(), f'{GNN_SNAPSHOT_PREFIX}{m_n}_E3.pth')
...
df = add_attributes(df, TEST_JSONL_PATHS[0] if TEST_JSONL_PATHS else "dummy_test")
...
def run_training_pipeline(
    ...
    eval_graph.n_id = torch.arange(eval_graph.num_nodes)
    flag = torch.tensor([True] * eval_graph.num_nodes, dtype=torch.bool)
    ...
    for m_n in range(num_snapshots):
        ...
        for subg in loader:
            ...
            pred = indices[:, 0]
            cond = (pred == subg.y)
            flag[subg.n_id[cond]] = torch.logical_and(
                flag[subg.n_id[cond]],
...
for m_n in range(NUM_SNAPSHOTS):
  model.load_state_dict(torch.load(f'{GNN_SNAPSHOT_PREFIX}{m_n}_E3.pth'))
  loader = NeighborLoader(graph, num_neighbors=[-1,-1], batch_size=GNN_BATCH_SIZE)    
  for subg in loader:
      model.eval()
      out = model(subg.x, subg.edge_index)
...
if TRAIN_JSONL_PATHS:
    result = run_training_pipeline()
    pipeline_time = _time.time() - pipeline_start
    
    if result:
        result['pipeline_elapsed_seconds'] = round(pipeline_time, 2)
        result['config'] = {
            'train_paths': TRAIN_JSONL_PATHS,
            'test_paths': TEST_JSONL_PATHS,
        ...
        _benchmark_file = f'benchmark_results_{_time.strftime("%Y%m%d_%H%M%S")}.json'
        with open(_benchmark_file, 'w') as _bf:
            _json.dump(result, _bf, indent=2)
        logger.info(f"Benchmark results saved to {_benchmark_file}")
else:
    logger.warning("")
    logger.warning("═" * 60)
    logger.warning("  No JSONL paths configured!")
    logger.warning(f"  Set TRAIN_JSONL_PATHS = ['path/to/your/file.jsonl']")
    logger.warning("  in the CONFIGURATION cell and re-run.")
    logger.warning("═" * 60)
    logger.warning("")
    logger.info("Running synthetic data for smoke test instead...")
    logger.info("Set USE_SYNTHETIC = True and re-run for synthetic mode.")
